 ...
 */
package com.kingoit.ar.util.math;
...
//	public final static int NAUTICAL_MILES = 2;
    ...
    public static final double EARTH_RADIUS = 6378137.0;// m
    ...
    public static double calculateDistanceMeters(double aLong, double aLat, double bLong, double bLat) {
        Log.e("渲染距离", "aLong：" + aLong + ",aLat" + aLat + ",bLong" + bLong + ",bLat" + bLat);
        double d2r = (Math.PI / 180);

        double Lat1 = aLat * d2r;
        double Lat2 = bLat * d2r;
        double dLat = Lat1 - Lat2;
        double dLon = aLong * d2r - bLong * d2r;
		double a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(aLat * d2r) * Math.cos(bLat * d2r)
				* Math.sin(dLon / 2) * Math.sin(dLon / 2);
		double c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
		return EARTH_RADIUS_KM * c * 1000;
//        double c = 2 * Math.asin(Math.sqrt(Math.pow(Math.sin(dLat / 2), 2)
